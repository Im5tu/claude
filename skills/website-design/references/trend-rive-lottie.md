...
Both runtimes ship framework-neutral packages — wrap them in a **SolidJS island** hydrated `client:visible`. Never use `@rive-app/react-canvas` or `lottie-react` — pick the vanilla packages:

- **Rive:** `@rive-app/canvas` (or `@rive-app/webgl2`). Instantiate in `onMount` with `new Rive({ src, stateMachines, autoplay })` and mutate state via `rive.stateMachineInputs(…)`. Call `rive.cleanup()` in `onCleanup`.
- **Lottie:** `lottie-web`. Call `lottie.loadAnimation({ container, path, loop, autoplay })` in `onMount`, `anim.destroy()` in `onCleanup`.

Guard with `matchMedia("(prefers-reduced-motion: reduce)").matches` — if true, render the poster/static frame and skip initialisation.
...
Cross-aesthetic applications: A light + moderate + technical direction can use Rive state-machine icons in the navigation — hover states on nav items trigger brand-specific micro-animations that reinforce product identity beyond what CSS can express. A warm + restrained + texture-high direction can use Lottie illustrations as gentle ambient loops in the hero — hand-drawn style animation matches the craft aesthetic and adds life to the page. A dark + expressive + editorial-display direction can use Rive interactive illustrations as a portfolio navigation mechanic — each project thumbnail is a living preview that responds to cursor proximity.
