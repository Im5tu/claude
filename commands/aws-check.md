...
6. **Detect log format.** For each log group, fetch a small sample (one or two events) to determine whether logs are JSON-structured or plain text. If the message body parses as JSON and contains a `level` property, treat the group as JSON-formatted. Otherwise treat it as plain text.
